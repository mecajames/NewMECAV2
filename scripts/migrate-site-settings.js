/**
 * Migrate site_settings from remote to local Supabase
 */
...
async function main() {
  console.log('🚀 Migrating site_settings from remote to local...\n');

  try {
    console.log('📥 Fetching from remote...');
    const settings = await httpsGet(REMOTE_URL, '/rest/v1/site_settings?select=*', REMOTE_KEY);

    if (!settings || settings.length === 0) {
      console.log('⚠️  No site_settings found in remote database');
      return;
    }
    ...
    await httpPost(LOCAL_URL, LOCAL_PORT, '/rest/v1/site_settings', LOCAL_SERVICE_KEY, settings);

    console.log('✅ Successfully migrated site_settings!');
    console.log('\n🌐 View your site at: http://localhost:5176/');
