/**
 * Migrate data from correct remote Supabase (qykahrgwtktqycfgxqep) to local
 */
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          resolve(data ? (data.length > 0 ? JSON.parse(data) : {}) : {});
        } else {
          reject(new Error(`HTTP ${res.statusCode}: ${data}`));
        }
      });
...
async function migrateTable(tableName) {
  ...
  try {
    // Fetch from remote
    console.log(`   Fetching from https://${REMOTE_URL}/rest/v1/${tableName}...`);
