...
export const CHECKLIST_SECTIONS: ChecklistSectionData[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'admin-create-member', title: '10.2 Create a New Member', steps: [
        'Click "Create New Member" on the Members page',
        'Step through the wizard: pick account type (Member vs Basic) — Basic is for guest registrations being converted',
        'Fill in first/last name, email, and phone',
        'Choose a password option: auto-generate, set manually, or send-reset-link',
        'If applicable, tick "Force password change on first login"',
        'Optionally assign a membership type and pick a payment method (test purchase, manual entry, or comp)',
        'Save and confirm the new member appears in the Members list with the right MECA ID, status, and contact info',
      ], expectedResult: 'New member is fully created with profile, auth account, and (if chosen) an active membership.' },
      { id: 'admin-edit-member', title: '10.3 Edit a Member', steps: [
        'Open a member\'s detail page',
        'Edit each section: profile (name/contact), addresses (billing/shipping), staff/role flags',
        'Save each section and confirm the change persists after a refresh',
        'Try editing the MECA ID — confirm only super-admin emails can change it',
      ], expectedResult: 'Every section of the member record is editable; protected fields (MECA ID) are properly restricted.' },
      ...
      { id: 'admin-manual-cash-payment', title: '10.7 Apply a Manual Cash / Check Payment', steps: [
        'Open the member detail page',
        'Find the section for recording a manual payment (membership or invoice)',
        'Pick payment method "Cash" (or "Check") and enter the amount + a reference note (e.g. check number)',
        'Optionally attach the payment to a specific membership or invoice',
        'Save and confirm a new entry appears in the member\'s billing history with method = cash/check',
        'Open Admin > Billing > Orders or Invoices and confirm the same payment is reflected there',
        'If QuickBooks sync is connected, verify the entry mirrors over within the next sync window',
      ], expectedResult: 'The payment is recorded against the right account, shows in billing history, and reaches QuickBooks.' },
      { id: 'admin-update-billing-payment', title: '10.8 Update a Member\'s Billing / Payment Method', steps: [
        'Open a member\'s detail page',
        'Update the billing address (street/city/state/zip/country) and save',
        'Confirm the new address appears on subsequent invoices and on the public profile (if applicable)',
        'If the member has a saved payment method (card on file), use the admin tool to update or remove it',
        'Confirm the change reflects in their Stripe billing portal next time they open it',
      ], expectedResult: 'Billing address and saved payment method update correctly and propagate to Stripe.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'admin-promote-judge', title: '13.7 Promote a Member to Judge', steps: [
        'Open a member\'s detail page',
        'Grant Judge permission via the Permissions panel (sets can_apply_judge = true)',
        'As that member (or via impersonation), navigate to Judges > Apply and submit a Judge application',
        'Back as admin, go to Admin > Judge Applications and approve the application',
        'Confirm a judges record is created and the member appears in Admin > Manage Judges and the public Judges directory',
        'Optionally assign the new judge to a test event and verify they appear on its judging panel',
      ], expectedResult: 'Member is fully promoted to active Judge with a record, public listing, and event-assignment capability.' },
      { id: 'admin-promote-ed', title: '13.8 Promote a Member to Event Director', steps: [
        'Open a member\'s detail page',
        'Grant Event Director permission via the Permissions panel',
        'As that member (or via impersonation), navigate to Event Directors > Apply and submit',
        'Back as admin, go to Admin > ED Applications and approve',
        'Confirm an event_directors record is created and the member appears in Admin > Manage Event Directors and the public ED directory',
        'Try assigning the new ED to host a test event and verify they appear on the event detail page',
      ], expectedResult: 'Member is fully promoted to active Event Director and can be assigned to host events.' },
    ],
  },
  {
