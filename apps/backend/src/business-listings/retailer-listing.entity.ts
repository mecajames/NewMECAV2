...
@Entity({ tableName: 'retailer_listings', schema: 'public' })
export class RetailerListing {
  ...
  // Special offer/discount/coupon text for MECA members
  @Property({ type: 'text', nullable: true, fieldName: 'offer_text' })
  offerText?: string;
  ...
  // Store type
  @Property({ type: 'varchar', length: 50, fieldName: 'store_type', default: 'both' })
  storeType: string = StoreType.BOTH;
  ...
  // Pending edits awaiting admin review. When the owner edits an already-
  // approved listing, the new values are stashed here while the live row
  // continues to serve the public directory unchanged. Admin moderation
  // either merges these onto the row (approve) or clears them (reject).
