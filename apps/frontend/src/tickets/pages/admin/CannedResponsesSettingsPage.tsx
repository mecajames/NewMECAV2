...
import { ArrowLeft, Plus, Save, Trash2, Edit2, X, Share2, Lock, Loader2, AlertCircle, Search } from 'lucide-react';
 ...
 * stored server-side keyed by the owning agent; shared ones surface
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white mb-2">Canned Responses</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {Array.from(grouped.entries()).map(([category, items]) => (
              <div key={category}>
                <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-2">{category}</h2>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-between bg-slate-800 rounded-lg p-3 border border-slate-700">
            <div>
              <p className="text-white text-sm font-medium">Share with team</p>
              <p className="text-gray-400 text-xs">Let other support staff use this template (read-only for them).</p>
            </div>
            <button
              type="button"
              role="switch"
