...
import {
  createEventDirectorApplication,
  getMyEventDirectorApplication,
...
const STEPS: ApplicationStep[] = ['personal', 'location', 'experience', 'essays', 'references', 'acknowledgments', 'review'];
...
export default function EventDirectorApplicationPage() {
  ...
  async function checkExistingApplication() {
    setLoading(true);
    try {
      const [application, edProfile] = await Promise.all([
        getMyEventDirectorApplication(),
        getMyEventDirectorProfile(),
      ]);

      if (edProfile) {
        setIsEventDirector(true);
      } else if (application) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const dto: CreateEventDirectorApplicationDto = {
        ...formData,
      ...
      navigate('/dashboard', { state: { message: 'Event Director application submitted successfully!' } });
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-8">
        <div className="max-w-2xl mx-auto bg-slate-800 rounded-lg p-8 text-center">
          <h1 className="text-2xl font-bold text-white mb-4">You're Already an Event Director!</h1>
          <p className="text-slate-300 mb-6">
            You're already registered as a MECA Event Director. View your dashboard to see your events and stats.
          </p>
          <button
            onClick={() => navigate('/dashboard')}
        ...
        <div className="max-w-2xl mx-auto bg-slate-800 rounded-lg p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-slate-400">
              Submitted on {new Date(existingApplication.applicationDate || existingApplication.application_date).toLocaleDateString()}
            </span>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 py-8">
      <div className="max-w-4xl mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Event Director Application</h1>
          <p className="text-slate-400">
            Apply to become a MECA certified Event Director. Complete all sections below.
          </p>
        </div>
        ...
        <div className="bg-slate-800 rounded-lg p-6">
          ...
          {currentStep === 'experience' && (
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-slate-300 text-sm mb-1">Event Management Experience *</label>
                <textarea
                  value={formData.event_management_experience}
                  onChange={(e) => handleInputChange('event_management_experience', e.target.value)}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-orange-500 focus:outline-none h-24"
                  placeholder="Describe your experience managing events, competitions, or similar activities..."
                  required
                />
              </div>

              <div>
                <label className="block text-slate-300 text-sm mb-1">Team Management Experience *</label>
                <textarea
                  value={formData.team_management_experience}
                  onChange={(e) => handleInputChange('team_management_experience', e.target.value)}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-orange-500 focus:outline-none h-24"
                  placeholder="Describe your experience leading and managing teams..."
                  required
                />
              </div>

              <div>
                <label className="block text-slate-300 text-sm mb-1">Equipment & Resources</label>
                <textarea
                  value={formData.equipment_resources}
                  onChange={(e) => handleInputChange('equipment_resources', e.target.value)}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-orange-500 focus:outline-none h-24"
                  placeholder="Describe any equipment or resources you have access to for running events..."
                />
              </div>
            </div>
          )}
          ...
          {currentStep === 'essays' && (
            <div className="space-y-4">
              <div>
                <label className="block text-slate-300 text-sm mb-1">Why do you want to become a MECA Event Director? *</label>
                <textarea
                  value={formData.essay_why_ed}
                  onChange={(e) => handleInputChange('essay_why_ed', e.target.value)}
                  ...
                  required
                />
                <span className="text-xs text-slate-400">{formData.essay_why_ed.length} / 100 min characters</span>
              </div>

              <div>
                <label className="block text-slate-300 text-sm mb-1">What qualifies you to be a MECA Event Director? *</label>
                <textarea
            ...
            <div className="space-y-4">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-slate-300 text-sm">
                  I understand that MECA Event Directors are independent contractors and not employees of MECA. *
                </span>
              </label>
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                />
                <span className="text-slate-300 text-sm">
                  I agree to abide by the MECA Event Director Code of Conduct and maintain professional standards. *
                </span>
              </label>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-700/50 p-4 rounded-lg">
                ...
                <ul className="text-slate-400 text-sm space-y-1">
                  <li>{formData.ack_independent_contractor ? '✓' : '✗'} Independent Contractor Agreement</li>
                  <li>{formData.ack_code_of_conduct ? '✓' : '✗'} Code of Conduct</li>
                  <li>{formData.ack_background_check ? '✓' : '✗'} Background Check Consent</li>
                  <li>{formData.ack_terms_conditions ? '✓' : '✗'} Terms and Conditions</li>
                </ul>
              </div>
            </div>
          )}
