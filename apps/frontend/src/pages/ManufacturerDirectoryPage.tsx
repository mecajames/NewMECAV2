  ...
  const loadListings = async () => {
    try {
      const [listingsRes, featuredRes] = await Promise.all([
        directoriesApi.getAll('manufacturer'),
        directoriesApi.getFeatured('manufacturer'),
      ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-16 relative">
          <div className="max-w-3xl">
            <h1 className="text-5xl font-bold text-white mb-4">Manufacturer Directory</h1>
            <p className="text-xl text-gray-300">
              Official MECA manufacturers and industry-leading sponsors providing cutting-edge car audio technology
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {featured.length > 0 && (
          <div className="mb-16">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-3xl font-bold text-white">Featured Manufacturers</h2>
            </div>
        ...
        {/* All Manufacturers */}
        <div>
          <h2 className="text-3xl font-bold text-white mb-6">All Manufacturers</h2>
          <div className="grid md:grid-cols-2 gap-6">
        ...
        {filteredListings.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-lg">No manufacturers found matching your search.</p>
          </div>
        )}
      </div>
    </div>
...
function ListingCard({ listing, featured = false }: { listing: any; featured?: boolean }) {
  return (
    <div className={`bg-slate-800 rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition transform hover:-translate-y-1 ${featured ? 'border-2 border-orange-500' : ''}`}>
      {listing.bannerUrl && (
        <img src={listing.bannerUrl} alt={listing.businessName} className="w-full h-64 object-cover" />
      )}
      <div className="p-6">
        {listing.logoUrl && (
          <img src={listing.logoUrl} alt={listing.businessName} className="h-20 mb-4" />
        )}
        <h3 className="text-2xl font-bold mb-3 text-white">{listing.businessName}</h3>
