...
export const competitionResultsApi = {
  getResultsByEvent: async (eventId: string): Promise<CompetitionResultData[]> => {
    const response = await fetch(`${API_BASE_URL}/api/competition-results/event/${eventId}`);
  ...
  getLeaderboard: async (eventId: string): Promise<CompetitionResultData[]> => {
    const response = await fetch(`${API_BASE_URL}/api/competition-results/event/${eventId}/leaderboard`);
    if (!response.ok) throw new Error('Failed to fetch leaderboard');
    return response.json();
  },

  getResultsByCategory: async (eventId: string, category: string): Promise<CompetitionResultData[]> => {
    const response = await fetch(`${API_BASE_URL}/api/competition-results/event/${eventId}/category/${category}`);
  ...
  getResultsByCompetitor: async (competitorId: string): Promise<CompetitionResultData[]> => {
    const response = await fetch(`${API_BASE_URL}/api/competition-results/competitor/${competitorId}`);
