...
export const eventRegistrationsApi = {
  getRegistrations: async (page: number = 1, limit: number = 10): Promise<EventRegistrationData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations?page=${page}&limit=${limit}`);
    if (!response.ok) throw new Error('Failed to fetch registrations');
    return response.json();
  },

  getRegistration: async (id: string): Promise<EventRegistrationData> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations/${id}`);
  ...
  getRegistrationsByEvent: async (eventId: string): Promise<EventRegistrationData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations/event/${eventId}`);
  ...
  countRegistrationsByEvent: async (eventId: string): Promise<{ count: number }> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations/event/${eventId}/count`);
  ...
  getRegistrationsByUser: async (userId: string): Promise<EventRegistrationData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations/user/${userId}`);
  ...
  createRegistration: async (data: Partial<EventRegistrationData>): Promise<EventRegistrationData> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/event-registrations`, {
      method: 'POST',
