...
export const eventsApi = {
  getEvents: async (page: number = 1, limit: number = 10): Promise<EventData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/events?page=${page}&limit=${limit}`);
  ...
  getUpcomingEvents: async (): Promise<EventData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/events/upcoming`);
    if (!response.ok) throw new Error('Failed to fetch upcoming events');
    return response.json();
  },

  getEventsByStatus: async (status: string): Promise<EventData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/events/status/${status}`);
  ...
  getEventsByDirector: async (directorId: string): Promise<EventData[]> => {
    const response = await authenticatedFetch(`${API_BASE_URL}/api/events/director/${directorId}`);
